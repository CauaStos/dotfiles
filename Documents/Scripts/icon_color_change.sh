color_distance() {
    local input=($1 $2 $3)
    local color=($4 $5 $6)
    local distance=$(( (${input[0]} - ${color[0]})**2 + (${input[1]} - ${color[1]})**2 + (${input[2]} - ${color[2]})**2 ))
    echo $distance
}

get_closest_color() {
    local input_rgb=($1 $2 $3)
    ...
    for color in "${colors[@]}"; do
        IFS=':' read -r color_name r g b <<< "$color"
        distance=$(color_distance "${input_rgb[@]}" "$r" "$g" "$b")
